...
document.addEventListener('DOMContentLoaded', () => {
  ...
  form.addEventListener('submit', (e) => {
    ...
    if(isConfirmPasswordInvalid) return;
...
confirmPassword.addEventListener("blur", () => {
  ...
  isConfirmPasswordInvalid = showError(confirmPassword, confirmError, confErrors);
});
...
const ssnError = document.getElementById("ssnerror"); // if you want to use it
...
function validateEmail() {
    ...
    if (!emailPattern.test(email)) {
        errorDiv.textContent = "Invalid email format. Use name@domain.tld";
        return false;
    }
...
async function loadStates() {
    try {
        // dynamically import the states from the JS file
